...
#define __SYCL_SWIZZLE_MIXIN_METHOD_NON_CONST(COND, NAME, ...)                 \
  ...
  std::enable_if_t<                                                            \
      (COND), std::conditional_t<SwizzleResult::size() == 1,                   \
                                 decltype(std::declval<SwizzleResult>()[0]),   \
                                 SwizzleResult>>                               \
  NAME() {                                                                     \
    auto sw = static_cast<Self_ *>(this)->template swizzle<__VA_ARGS__>();     \
    if constexpr (sw.size() == 1)                                              \
      return sw[0];                                                            \
    else                                                                       \
      return static_cast<Self_ *>(this)->template swizzle<__VA_ARGS__>();      \
  }
...
#define __SYCL_SWIZZLE_MIXIN_METHOD_CONST(COND, NAME, ...)                     \
  ...
  NAME() const {                                                               \
    auto sw =                                                                  \
        static_cast<const Self_ *>(this)->template swizzle<__VA_ARGS__>();     \
    if constexpr (sw.size() == 1)                                              \
      return sw[0];                                                            \
    else                                                                       \
...
template <typename Self, int NumElements> struct NamedSwizzlesMixInNonConst {
